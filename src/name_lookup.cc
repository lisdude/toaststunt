...
static pid_t
spawn_pipe(void (*child_proc) (int to_parent, int from_parent), int *to_child, int *from_child)
{
...
lookup(int to_intermediary, int from_intermediary)
{
	...
	for (;;) {
		...
		if (req.kind == request::REQ_ADDR_FROM_NAME) {
			...
			id = set_timer(req.timeout, timeout_proc, nullptr);
			if (getnameinfo
			    ((struct sockaddr *)&req.u.address, sizeof(req.u.address), hbuf, sizeof(hbuf), nullptr, 0,
			     0) == 0)
				host_name = hbuf;
			else
				host_name = "";
...
restart_lookup(void)
{
	...
	if (lookup_pid)
		...
		errlog("NAME_LOOKUP: Can't spawn lookup process; " "will try again later...\n");
}
		...
		if (lookup_pid) {	/* Only try to deal with lookup if alive */
			...
			if (req.kind == request::REQ_ADDR_FROM_NAME) {
				...
				if (robust_read(from_lookup, &len, sizeof(len))
					...
					if (len > 0 && robust_read(from_lookup, buffer, len) != len) {
						restart_lookup();
						len = 0;
					}
				}
	...
	if (!dead_intermediary) {
		...
		if (write(to_intermediary, &req, sizeof(req)) != sizeof(req))
			...
			if (robust_read(from_intermediary, buffer, len) != len)
				abandon_intermediary("LOOKUP_NAME: " "Data-read from intermediary failed");
			else {
				uint32_t a = ntohl(addr->sin_addr.s_addr);
				sprintf((char *)buffer + len, " [%u.%u.%u.%u]", (unsigned)(a >> 24) & 0xff,
